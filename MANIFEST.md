...
scripts/
  ...
  state.py                             # Pipeline checkpoints + crash recovery in vault state dir
  ...
  summarize.py                         # Distills articles via Ollama (mid/full) or file output for Haiku
...
- `plugin.json` declares the plugin — lists both skills and all 4 agents for Claude Code discovery
- `skills/research/SKILL.md` is the sole entry point — orchestrates all 9 stages, dispatches all 4 agents via the Task tool, and writes final notes itself (Sonnet) or escalates synthesis notes to Opus
- `state.py` enables crash recovery — checkpoints after every stage to `{vault}/.research-workflow/state/`; skill can resume, restart, or abandon any run
...
- `vault_index.py` is shared by classify-agent, thread-discoverer, and topic-resolver — FTS5 queries replace glob-all-markdown; rebuilt incrementally at Stage 0
...
- `summarize.py` branches on tier: `--model` flag sends to Ollama; `--prepare-for-claude` writes per-article files for Haiku subagents
