...
skills/
  researcher/SKILL.md                  # Sonnet orchestrator — multi-hop pipeline (triage → resolve → hop loop → quality gate → classify → write → wikilinks → discover → analyze → complete)
...
scripts/
  ...
  migrate.py                           # .env→config.json migration; vault folder renames; .research-workflow→.researcher dir migration
...
tests/
  ...
  test_skill_contract.py               # Pins load-bearing SKILL.md shapes (text-level assertions)
  test_manifest_budget.py              # Guards this file's line/word budget
...
- `skills/researcher/SKILL.md` is the sole entry point — orchestrates pipeline, dispatches all 7 agents via Task tool, writes notes (Sonnet) or escalates to Opus
...
- `migrate.migrate_vault_dir` runs at Stage 0a (before config load) — one-time `.research-workflow/`→`.researcher/` rename for existing vaults
- **v3.1 case learning:** `case_analyzer.analyze()` is dispatched from `skills/researcher/SKILL.md:Stage 10d` and wires `pattern_detection.py` (heuristics), `accumulator.py` (candidates), `learned_patterns.py` (graduated), and `score_updates.py` (W/L scoring)
- **v3.1 vault state:** `accumulator.json` + `learned_patterns.md` live at `{vault}/.researcher/`; `state.acquire_state_lock` serializes Stage 10d/10e writes so concurrent /researcher runs can't race
